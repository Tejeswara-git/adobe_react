  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        page: '/workshops',
        subItems: [
          { label: 'Upcoming Workshops', href: '#/workshops/upcoming', page: '/workshops/upcoming' },
          { label: 'Past Workshops', href: '#/workshops/past', page: '/workshops/past' },
        ],
      },
      ...
      >
        <div className="nav-container">
          ...
          <div className={`nav-links ${navOpen ? 'open' : ''}`} id="nav-links">
            ...
            {dropdownMenus.map((menu) => (
              ...
              >
                <div className={`nav-link nav-dropdown-toggle ${isDropdownActive(menu) ? 'active' : ''}`}>
                  <a href={menu.href} onClick={() => setNavOpen(false)}>
                    {menu.label}
                  </a>
                  <button
                    type="button"
                ...
                <div className="nav-dropdown-menu">
                  {menu.items.map((item) =>
                    item.subItems ? (
                      <div key={item.page} className="nav-flyout-wrapper">
                        <span className="nav-dropdown-item nav-flyout-label">
                          {item.label} <span className="nav-flyout-arrow">›</span>
                        </span>
                        <div className="nav-flyout-menu">
                          {item.subItems.map((sub) => (
                            <a
                              key={sub.page}
                              href={sub.href}
                              className={`nav-dropdown-item ${currentPage === sub.page ? 'active' : ''}`}
                              onClick={() => { setNavOpen(false); setOpenDropdown(null); }}
                            >
                              {sub.label}
                            </a>
                          ))}
            ...
            <a href="#contact" className="nav-link" id="nav-volunteer" onClick={() => setNavOpen(false)}>
              Join as a Volunteer
            </a>
          </div>
